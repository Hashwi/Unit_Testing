...
describe("Greeting Components", () => {
  ...
  test("renders Nice to see you here if the button was NOT clicked", () => {
    ...
    // Assert
    const outputElement = screen.getByText("Nice to see you here!!", {
      exact: false,
    });
  ...
  test('does not render Nice to see you here if the button was  clicked', ()=> {
    //Arrange
    render(<Greeting/>)
    ...
    // Assert
    const outputElement = screen.queryByText("Nice to see you here", { exact: false });
